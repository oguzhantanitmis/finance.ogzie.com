...
import { Landmark, CreditCard, Users, Clock } from 'lucide-react'
...
const TYPE_ACCENT: Record<DebtType, string> = { loan: 'info', card: 'danger', kmh: 'warning', personal: 'purple' }
...
export default function DebtsMobile({ debts }: { debts: DebtItem[] }) {
  ...
  const byType = (['card', 'loan', 'kmh', 'personal'] as DebtType[])
    .map((t) => ({ value: debts.filter((x) => x.type === t).reduce((a, b) => a + b.remaining, 0), color: TYPE_ACCENT[t] as any, label: TYPE_LABEL[t] }))
    .filter((x) => x.value > 0)
