...
export default function ReportsMobile({ data }: { data: ReportSeries }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2.5">
        <button className="btn-secondary flex-1" onClick={() => window.open('/api/export/reports?format=csv', '_blank')}><List className="w-4 h-4" /> CSV indir</button>
        <button className="btn-secondary flex-1" onClick={() => window.open('/api/export/reports?format=pdf', '_blank')}><ReceiptText className="w-4 h-4" /> PDF rapor</button>
      </div>
    </div>
  )
}
